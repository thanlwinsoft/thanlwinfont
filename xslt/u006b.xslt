...
<xsl:template match="svg:g">
	...
	<xsl:call-template name="u006b"/>
	</xsl:copy>
</xsl:template>

<xsl:template name="u006b">
	<xsl:param name="xOffset" select="0"/>
	<xsl:param name="yOffset" select="0"/>
	<xsl:variable name="kAngle" select="math:atan(2 * $waYOuterRadius div (2 * $waXOuterRadius - math:sqrt(2) * $thickness))"/>
    ...
    <xsl:call-template name="line">
        ...
        <xsl:with-param name="y" select="-($latinAscent - $waYOuterRadius - $dyIntersect)"/>
    </xsl:call-template>
    <xsl:call-template name="line">
    	<xsl:with-param name="x" select="2 * $waXOuterRadius - $thickness - $dxLeg"/>
        <xsl:with-param name="y" select="$waYOuterRadius - $dyIntersect"/>
    </xsl:call-template>
    <xsl:call-template name="line">
