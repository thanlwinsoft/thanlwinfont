...
<xsl:template match="svg:g">
	...
	<xsl:call-template name="u0072"/>
	</xsl:copy>
</xsl:template>


<xsl:template name="u0072">
	<xsl:param name="xOffset" select="0"/>
	<xsl:param name="yOffset" select="0"/>
	<xsl:variable name="intersectAngle" select="math:acos(($waXInnerRadius + $waXOuterRadius) div (2* $waXOuterRadius))"/>
	<xsl:element name="path" use-attribute-sets="pathAttribs">
	    <xsl:attribute name="d">
			...
			<xsl:call-template name="arc">
				<xsl:with-param name="rx" select="2 * $waXOuterRadius"/>
				<xsl:with-param name="ry" select="$waYOuterRadius"/>
				<xsl:with-param name="axisRotation" select="0"/>
				<xsl:with-param name="large" select="0"/>
				<xsl:with-param name="clockwise" select="0"/>
				<xsl:with-param name="x" select="2 * $waXOuterRadius *(math:cos($intersectAngle))"/>
				<xsl:with-param name="y" select="(1 - math:sin($intersectAngle)) * $waYOuterRadius"/>
			</xsl:call-template>
			<xsl:call-template name="line">
				<xsl:with-param name="x" select="0"/>
			...
			<xsl:call-template name="arc">
				<xsl:with-param name="rx" select="$waXInnerRadius  + $waXOuterRadius"/>
