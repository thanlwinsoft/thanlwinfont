...
<xsl:template match="svg:g">
	...
	<xsl:call-template name="u0071"/>
	</xsl:copy>
</xsl:template>


<xsl:template name="u0071">
	...
	<xsl:element name="path" use-attribute-sets="pathAttribs">
	    <xsl:attribute name="d">
			...
			<xsl:call-template name="line">
				<xsl:with-param name="x" select="0"/>
				<xsl:with-param name="y" select="-(1 - math:sin($intersectAngle)) * $waYOuterRadius - $latinDescent"/>
			</xsl:call-template>
			<xsl:call-template name="line">
				<xsl:with-param name="x" select="$thickness"/>
				<xsl:with-param name="y" select="0"/>
			...
			<xsl:call-template name="line">
				<xsl:with-param name="x" select="0"/>
				<xsl:with-param name="y" select="2 * $waYOuterRadius + $latinDescent"/>
			</xsl:call-template>
