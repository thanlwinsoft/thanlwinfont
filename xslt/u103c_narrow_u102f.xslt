...
<xsl:template match="svg:g">
	...
	<xsl:call-template name="u103c_narrow_u102f"/>
	</xsl:copy>
</xsl:template>

<xsl:template name="u103c_narrow_u102f">
	<xsl:param name="xOffset" select="0"/>
	<xsl:param name="yOffset" select="0"/>
    <xsl:element name="path" use-attribute-sets="pathAttribs">
    <xsl:attribute name="d">
    <xsl:call-template name="Move">
        <xsl:with-param name="x" select="$xOffset + 2 * $waXOuterRadius +2 * $preGuard + $thickness + $postGuard"/>
	...
	<xsl:call-template name="corner">
		...
		<xsl:with-param name="nextX" select="-$waXOuterRadius - $preGuard"/>
	...
	<xsl:call-template name="corner">
		<xsl:with-param name="x" select="$waXOuterRadius + $preGuard - $thickness"/>
        <xsl:with-param name="y" select="0"/>
		<xsl:with-param name="r" select="$cornerInnerRadius"/>
		<xsl:with-param name="nextX" select="0"/>
