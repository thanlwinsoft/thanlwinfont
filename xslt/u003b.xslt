...
<xsl:template name="u003b">
    ...
    <xsl:call-template name="arc">
        ...
        <xsl:with-param name="ry" select="2.5 * $latinDotRadius"/>
        <xsl:with-param name="axisRotation" select="0"/>
        <xsl:with-param name="large" select="0"/>
        <xsl:with-param name="clockwise" select="0"/>
        <xsl:with-param name="x" select="- 2 * $latinDotRadius"/>
        <xsl:with-param name="y" select="- 2.5 * $latinDotRadius"/>
    </xsl:call-template>
    <xsl:call-template name="arc">
        <xsl:with-param name="rx" select="$latinDotRadius"/>
        <xsl:with-param name="ry" select="1.5 * $latinDotRadius"/>
        <xsl:with-param name="axisRotation" select="0"/>
        <xsl:with-param name="large" select="0"/>
        <xsl:with-param name="clockwise" select="1"/>
        <xsl:with-param name="x" select="$latinDotRadius"/>
        <xsl:with-param name="y" select="1.5*$latinDotRadius"/>
    </xsl:call-template>
<!--
    <xsl:text>l</xsl:text><xsl:value-of select="$latinDotRadius"/>
    <xsl:text>,</xsl:text><xsl:value-of select="2 * $latinDotRadius"/>
    -->
