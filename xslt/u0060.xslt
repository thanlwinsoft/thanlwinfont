...
<xsl:template name="u005e">
    ...
    <xsl:variable name="dx" select="2 * $waXOuterRadius * $upperScale"/>
    <xsl:variable name="dy" select="2 * $waXOuterRadius * $upperScale"/>
    <xsl:variable name="lineAngle" select="$pi div 4"/>
    <xsl:element name="path" use-attribute-sets="pathAttribs">
    <xsl:attribute name="d">
    <xsl:call-template name="Move">
        <xsl:with-param name="x" select="$xOffset + $preGuard + $waXOuterRadius - .5 * $dx + .5 * $thickness div math:sin($lineAngle)"/>
        <xsl:with-param name="y" select="$yOffset + 2 * $waYOuterRadius + $upperPad + $dy"/>
    </xsl:call-template>
