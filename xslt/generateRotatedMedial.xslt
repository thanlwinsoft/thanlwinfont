...
<axsl:template match="svg:g">
	<axsl:copy>
		<axsl:attribute name="transform">
			<axsl:text><xsl:text>matrix(0  </xsl:text></axsl:text><axsl:value-of select="$medialScale"/>
			<axsl:text><xsl:text> </xsl:text></axsl:text>
			<axsl:value-of select="$medialScale"/><axsl:text><xsl:text> 0 </xsl:text></axsl:text>
...
<axsl:template name="{concat('u1039_', $base)}">
	...
	<axsl:call-template name="{$baseTemplate}">
		<axsl:with-param name="xOffset" select="-$preGuard + $medialPad div $medialScale"/>
		<axsl:with-param name="yOffset" select="-2*$waYOuterRadius - $postGuard div $medialScale"/>
	</axsl:call-template>
</axsl:template>
