...
<xsl:variable name="advance" select="round($preGuard + $thickness * (1 + math:sqrt(2)) + $postGuard)"/>
...
<xsl:template name="u0031">
    ...
    <xsl:call-template name="Move">
        ...
        <xsl:with-param name="y" select="$yOffset + $ascent - $thickness * math:sqrt(2)"/>
    </xsl:call-template>
    <xsl:text>l</xsl:text><xsl:value-of select="$thickness * math:sqrt(2)"/>
    <xsl:text>,</xsl:text><xsl:value-of select="$thickness * math:sqrt(2)"/>
