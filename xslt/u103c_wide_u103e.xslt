...
<xsl:template match="svg:g">
	...
	<xsl:call-template name="u103c_wide_u103e"/>
	</xsl:copy>
</xsl:template>

<xsl:template name="u103c_wide_u103e">
	...
	<xsl:call-template name="corner">
		<xsl:with-param name="x" select="-$cornerOuterRadius - 2 * $waXOuterRadius"/>
	...
	<xsl:call-template name="corner">
		<xsl:with-param name="x" select="2 * $waXOuterRadius - $thickness + $cornerOuterRadius"/>
        <xsl:with-param name="y" select="0"/>
		<xsl:with-param name="r" select="$cornerOuterRadius"/>
		<xsl:with-param name="nextX" select="0"/>
	...
	<xsl:call-template name="corner">
		...
		<xsl:with-param name="nextX" select="-2 * $waXOuterRadius + $thickness - $cornerInnerRadius"/>
        <xsl:with-param name="nextY" select="0"/>
	</xsl:call-template>
	<xsl:call-template name="corner">
	...
	<xsl:call-template name="corner">
		...
		<xsl:with-param name="r" select="$cornerInnerRadius"/>
		<xsl:with-param name="nextX" select="$cornerInnerRadius + 2 * $waXOuterRadius"/>
        <xsl:with-param name="nextY" select="0"/>
	</xsl:call-template>
	<xsl:call-template name="corner">
		<xsl:with-param name="x" select="$cornerInnerRadius + 2 * $waXOuterRadius - $thickness"/>
        <xsl:with-param name="y" select="0"/>
		<xsl:with-param name="r" select="$cornerInnerRadius"/>
		<xsl:with-param name="nextX" select="0"/>
	...
	<xsl:call-template name="Move">
        <xsl:with-param name="x" select="$xOffset + 3*$waXOuterRadius + $cornerOuterRadius + $preGuard + $thickness"/>
