...
<axsl:variable name="advance" select="$widthOffset + $preGuard + 2 * $thickness + 2 * $postGuard"/>
...
<axsl:template match="svg:g">
	<axsl:copy use-attribute-sets="gAttribs">
		<axsl:call-template name="{concat($base, '_u103c_', $upperVowel)}"/>
	</axsl:copy>
</axsl:template>

<axsl:template name="{concat($base, '_u103c_', $upperVowel)}">
