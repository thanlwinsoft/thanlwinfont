...
<xsl:template name="u0059">
    ...
    <xsl:variable name="lineAngle" select="math:atan(.5 * $latinAscent div ($waXOuterRadius - .5 * $thickness))"/>
    ...
    <xsl:call-template name="line">
        <xsl:with-param name="x" select="-($waXOuterRadius - $thickness * ( 1 div math:sin($lineAngle)))"/>
        <xsl:with-param name="y" select="- math:tan($lineAngle) * ($waXOuterRadius - $thickness * (1 div math:sin($lineAngle)))"/>
    </xsl:call-template>
     <xsl:call-template name="line">
        <xsl:with-param name="x" select="-($waXOuterRadius - $thickness * ( 1 div math:sin($lineAngle)))"/>
        <xsl:with-param name="y" select="math:tan($lineAngle) * ($waXOuterRadius - $thickness * (1 div math:sin($lineAngle)))"/>
