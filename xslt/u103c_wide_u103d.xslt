...
<xsl:template match="svg:g">
	...
	<xsl:call-template name="u103c_wide_u103d"/>
	</xsl:copy>
</xsl:template>

<xsl:template name="u103c_wide_u103d">
    ...
    <xsl:call-template name="Move">
        <xsl:with-param name="x" select="$xOffset + 4 * $waXOuterRadius +$preGuard + $thickness + $postGuard"/>
	...
	<xsl:call-template name="corner">
		<xsl:with-param name="x" select="-$postGuard - 2 * $waXOuterRadius - $thickness"/>
	...
	<xsl:call-template name="corner">
		...
		<xsl:with-param name="r" select="$cornerOuterRadius"/>
		<xsl:with-param name="nextX" select="3 * $waXOuterRadius + $postGuard + $thickness"/>
        <xsl:with-param name="nextY" select="0"/>
	</xsl:call-template>
	...
	<xsl:call-template name="corner">
		<xsl:with-param name="x" select="-3 * $waXOuterRadius - $cornerInnerRadius + $waXOuterRadius * $medialScale * math:sin($intersectAngle)"/>
        <xsl:with-param name="y" select="0"/>
	...
	<xsl:call-template name="corner">
		...
		<xsl:with-param name="r" select="$cornerInnerRadius"/>
		<xsl:with-param name="nextX" select="$postGuard + 2 * $waXOuterRadius"/>
        <xsl:with-param name="nextY" select="0"/>
	</xsl:call-template>
	<xsl:call-template name="corner">
	...
	<xsl:call-template name="Move">
        <xsl:with-param name="x" select="$xOffset + 3*$waXOuterRadius + $preGuard + $postGuard + $thickness"/>
