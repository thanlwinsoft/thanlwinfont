...
<xsl:variable name="advance" select="$narrowConsWidth + round(2 * $wideBelowDx)"/>
...
<xsl:template match="svg:g">
	...
	<xsl:call-template name="u102c">
		<xsl:with-param name="xOffset" select="$narrowConsWidth+$wideBelowDx"/>
		<xsl:with-param name="yOffset" select="0"/>
	</xsl:call-template>
	</xsl:copy>
