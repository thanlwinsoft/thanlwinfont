...
<axsl:import href="../xslt/u1030_tall.xslt"/>
...
<axsl:variable name="advance" select="$preGuard + $postGuard + 5 * $thickness"/>
...
<axsl:template match="svg:g">
	...
	<axsl:call-template name="u1030_tall">
	</axsl:call-template>
	</axsl:copy>
</axsl:template>
...
<axsl:template name="{concat('u1039_', $base)}">
	...
	<axsl:call-template name="{$baseTemplate}">
		<axsl:with-param name="xOffset" select="($xOffset div $medialScale) -$widthOffset -$preGuard -$postGuard"/>
